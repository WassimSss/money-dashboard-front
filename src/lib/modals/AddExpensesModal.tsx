...
const AddExpensesModal: React.FC<AddExpensesModalProps> = ({ closeModal }) => {
    ...
    const handleSubmit = async () => {
        const responseAddExpenses = await addExpenses(user.token, amount, moment(date))
        console.log(responseAddExpenses)

        dispatch(setExpensesToStore(responseAddExpenses.expenses))
        if (responseAddExpenses.result) {
            toast.success('Revenu ajouté avec succès');
            closeModal();
        } else {
            toast.error(responseAddExpenses.message);
        }
    }
                        ...
                        <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div className="sm:flex sm:items-start">
                                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    ...
                                    <div className="mt-2">
                                        <label htmlFor="paymentDate" className="block text-sm font-medium text-gray-700">Date de la dépense</label>
